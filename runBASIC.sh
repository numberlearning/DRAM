...
for i in $(seq 1 10)
do
    MODELNAME="BASIC_run_${i}"
    mkdir "model_runs/$MODELNAME"
    sed -i '/model_name/c\model_name = "'$MODELNAME'"' basic_model_settings.py
    nohup python3 -u BASIC_classify_blobs.py $MODELNAME > "model_runs/$MODELNAME/nohup.out" &
done

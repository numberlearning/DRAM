...
task2 = tf.placeholder(tf.bool) # task2 state
...
## LOSE FUNCTION ################################ 
predcost = tf.cond(task2, lambda:tf.reduce_sum(pqs*mask_list[0]) / (num_list[0]+1), lambda:(tf.reduce_sum(cqs*mask_list[0]) + tf.reduce_sum(pqs*mask_list[0])) / (num_list[0]+1))
...
def evaluate():
    ...
    for i in range(batches_in_epoch):
        ...
        feed_dict = {task2: False, testing: True, x: nextX, onehot_labels: nextY, blob_list: nextZ, size_list: nextS, mask_list: nextM, num_list: nextN, count_word: nextC}
...
if __name__ == '__main__':
    ...
    for i in range(start_restore_index, train_iters):
        ...
        if i%2==0 and i<50000:#point_accuracy_prev < threshold:
            results = sess.run(fetches2, feed_dict = {task2: True,testing: False, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        elif i%2!=0 and i<50000:
            results = sess.run(fetches2, feed_dict = {task2: True,testing: True, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        elif i%2==0 and i>=50000:
            results = sess.run(fetches2, feed_dict = {task2: False,testing: False, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        else:
            results = sess.run(fetches2, feed_dict = {task2: False,testing: True, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
            results = sess.run(fetches2, feed_dict = {task2: False, testing: True, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        ...
        if i%100==0:
            if i==0:
                ...
                print("iter=%d : pot_accuracy: %f, Pc: %f" % (i, sum_pot_accuracy, sum_pc))
                point_accuracy_prev = sum_pot_accuracy 
            elif i!=0 and i<50000:
                print("iter=%d : pot_accuracy: %f, Pc: %f" % (i, sum_pot_accuracy/100, sum_pc/100))

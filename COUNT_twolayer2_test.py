...
def attn_window(scope, blob_list, h_point, N, glimpse, gx_prev, gy_prev, testing=None): 
...
## READ ## 
def read(x, h_point_prev, glimpse, testing=None):
...
def dense_to_one_hot(labels_dense, num_classes=output_size): # why?
...
def condition(x, count, counts, corrects, Rs, pointerxs, pointerys, teachers, cqs, pqs, h_point_prev, h_count_prev, point_state, count_state):
    zero_tensor = tf.cast(tf.zeros(1), tf.int64)
    return tf.equal(count,zero_tensor)[0]

def body(x, count, counts, corrects, Rs, pointerxs, pointerys, teachers, cqs, pqs, h_point_prev, h_count_prev, point_state, count_state):
    r, stats = read(x, h_point_prev, glimpse)
    predict_gx, predict_gy = stats
    ...
    # pointer
    pointer_x = predict_gx
    pointer_y = predict_gy
    teacher = [target_gx, target_gy]
    pointerxs.append(pointer_x[0,0])
    pointerys.append(pointer_y[0,0])
    ...
    # point reward
    in_blob_gx = tf.logical_and(tf.less(target_gx - max_edge/2, predict_gx), tf.less(predict_gx, target_gx + max_edge/2))
    in_blob_gy = tf.logical_and(tf.less(target_gy - max_edge/2, predict_gy), tf.less(predict_gy, target_gy + max_edge/2))
    in_blob = tf.logical_and(in_blob_gx, in_blob_gy)
    intensity = (predict_gx - target_gx)**2 + (predict_gy - target_gy)**2
    potquality = tf.where(in_blob, tf.reshape(tf.constant(1.0),[-1,1]), intensity)
    pq = tf.reduce_mean(potquality)
    pqs.append(pq)

    return (count, counts, corrects, Rs, pointerxs, pointerys, teachers, cqs, pqs, h_point_prev, h_count_prev, point_state, count_state)
...
while glimpse < glimpses:
    count, counts, corrects, Rs, pointerxs, pointerys, teachers, cqs, pqs, h_point_prev, h_count_prev, h_point_state, h_count_state = tf.while_loop(
          cond = condition,
          body = body,
          loop_vars=(x, count, counts, corrects, Rs, pointerxs, pointerys, teachers, cqs, pqs, h_point_prev, h_count_prev, point_state, count_state))
    glimpse += 1
 
## LOSE FUNCTION ################################ 
predcost = tf.reduce_mean(cqs) + tf.reduce_mean(pqs)

# all-knower
#R = tf.cast(tf.equal(correct, prediction), tf.float32)
reward = tf.reduce_mean(Rs)
...
def evaluate():
    ...
    for i in range(batches_in_epoch):
        nextX, nextY, nextZ, nextC = data.next_batch(batch_size)
        sumlabels += np.sum(nextY,0) 
        feed_dict = {x: nextX, onehot_labels: nextY, blob_list: nextZ, count_word: nextC}
        cs, r, cnt, cor, potx, poty, tch = sess.run([count_word, reward, counts, corrects, pointerxs, pointerys, teachers], feed_dict=feed_dict)
        accuracy += r
    ...
    print("COUNT: " + str(cnt))
    print("POINTER_X: " + str(potx))
    print("POINTER_Y: " + str(poty))
...
if __name__ == '__main__':
    ...
    for i in range(start_restore_index, train_iters):
        xtrain, ytrain, ztrain, ctrain = train_data.next_batch(batch_size)
        results = sess.run(fetches2, feed_dict = {x: xtrain, onehot_labels: ytrain, blob_list: ztrain, count_word: ctrain})

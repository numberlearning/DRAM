...
read_n = 8  # read glimpse grid width/height read_n>5 odd number
...
def filterbank(gx, gy, sigma2, delta, N):
    ...
    a = np.reshape([np.arange(dims[0])]*((N-1)*12+1), (((N-1)*12+1),-1))
    b = np.reshape([np.arange(dims[1])]*((N-1)*12+1), (((N-1)*12+1),-1))
    ...
    for i in range(1,N):
        for j in range(1,12+1):
            FFFx = tf.reshape(tf.exp(FFx[12*(i-1)+j]/(2*sigma2[i])),[1,-1])
            FFFy = tf.reshape(tf.exp(FFy[12*(i-1)+j]/(2*sigma2[i])),[1,-1]) 

...
task1 = tf.placeholder(tf.bool) # task1 state
...
for glimpse in range(glimpses):
    ...
    h_point, point_state = pointer(r, point_state)
    ...
    xx = [point_gx[0,0], target_gx]
    xxs.append(xx)
    yy = [point_gy[0,0], target_gy]
    yys.append(yy)
    ...
    for j in range(glimpses):
        in_range_x = tf.logical_and(tf.greater(predict_gx[0,0], blob_list[0][j][0] - size_list[0][j]/2 - 1), tf.less(predict_gx[0,0], blob_list[0][j][0] + size_list[0][j]/2 + 1))
        in_range_y = tf.logical_and(tf.greater(predict_gy[0,0], blob_list[0][j][1] - size_list[0][j]/2 - 1), tf.less(predict_gy[0,0], blob_list[0][j][1] + size_list[0][j]/2 + 1)) 
        in_range = tf.logical_and(in_range_x, in_range_y)
        blb_pot = tf.where(in_range, tf.constant(j), tf.constant(-1))
    
    blob_point.append(blb_pot)
    
## LOSE FUNCTION ################################ 
predcost = tf.cond(task1, lambda:tf.reduce_sum(cqs*mask_list[0]) / (num_list[0]+1), lambda:(tf.reduce_sum(cqs*mask_list[0]) + tf.reduce_sum(pqs*mask_list[0])) / (num_list[0]+1))
...
def evaluate():
    ...
    for i in range(batches_in_epoch):
        ...
        feed_dict = {task1: False, testing: True, x: nextX, onehot_labels: nextY, blob_list: nextZ, size_list: nextS, mask_list: nextM, num_list: nextN, count_word: nextC}
...
if __name__ == '__main__':
    ...
    fetches2=[]
    fetches2.extend([pointxs, pointys, predictxs, predictys, counts, corrects, count_accuracy, point_accuracy, predcost, train_op])
    ...
    for i in range(start_restore_index, train_iters):
        ...
        if i%2==0 and i<50000:#point_accuracy_prev < threshold:
            results = sess.run(fetches2, feed_dict = {task1: True, testing: False, x: bxtrain, onehot_labels: bytrain, blob_list: bztrain, size_list: bstrain, mask_list: bmtrain, num_list: bntrain, count_word: bctrain})
        elif i%2!=0 and i<50000:
            results = sess.run(fetches2, feed_dict = {task1: True, testing: True, x: bxtrain, onehot_labels: bytrain, blob_list: bztrain, size_list: bstrain, mask_list: bmtrain, num_list: bntrain, count_word: bctrain})
        elif i%2==0 and i>=50000:
            results = sess.run(fetches2, feed_dict = {task1: False, testing: False, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        else:
            results = sess.run(fetches2, feed_dict = {task1: False, testing: True, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        
        potxs_fetched, potys_fetched, prdxs_fetched, prdys_fetched, counts_fetched, corrects_fetched, count_accuracy_fetched, point_accuracy_fetched, predcost_fetched, _ = results
        ...
        if i%100==0:
            if i==0:
                # print("iter=%d : cnt_accuracy: %f, pot_accuracy: %f, Pc: %f" % (i, sum_cnt_accuracy, sum_pot_accuracy, sum_pc))
                print("iter=%d : cnt_accuracy: %f, Pc: %f" % (i, sum_cnt_accuracy, sum_pc))
                point_accuracy_prev = sum_pot_accuracy 
            elif i!=0 and i<50000:
                print("iter=%d : cnt_accuracy: %f, Pc: %f" % (i, sum_cnt_accuracy/100, sum_pc/100))

            else:
                # print("iter=%d : cnt_accuracy: %f, pot_accuracy: %f" % (i, count_accuracy_fetched, point_accuracy_fetched))

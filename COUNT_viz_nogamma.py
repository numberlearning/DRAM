...
# delta, sigma2
delta_1=max(dims[0],dims[1])*1.5/(read_n-1) # used to be *2
sigma2_1=delta_1*delta_1/4 # sigma=delta/2 
delta_2=max(dims[0],dims[1])/3/(read_n-1) # used to be /8
...
def read(x, h_point_prev, glimpse, testing):
    ...
    def filter_img(img, Fx_1, Fy_1, Fx_2, Fy_2, N):
        ...
        return tf.reshape(fimg, [batch_size, -1]) # fimg * tf.reshape(gamma, [-1,1])
...
## LOSE FUNCTION ################################ 
predcost = (tf.reduce_sum(cqs*mask_list[0]) + tf.reduce_sum(pqs*mask_list[0])) / (num_list[0]+1)
...
if __name__ == '__main__':
    ...
    for i in range(start_restore_index, train_iters+1):
        ...
        if i%2==0:#point_accuracy_prev < threshold:

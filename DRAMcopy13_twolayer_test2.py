#!/usr/bin/env py thon
# different onehot labels for input and output
import warnings
...
output_size_train = max_blobs_train - min_blobs_train + 1
output_size_test = max_blobs_test - min_blobs_test + 1
h_size = 400
...
onehot_labels_train = tf.placeholder(tf.float32, shape=(batch_size, output_size_train))
onehot_labels_test = tf.placeholder(tf.float32, shape=(batch_size, output_size_test))
...
with tf.variable_scope("output_train",reuse=REUSE):
    classification_train = tf.nn.softmax(linear(hidden, output_size_train))
with tf.variable_scope("output_test",reuse=REUSE):
    classification_test = tf.nn.softmax(linear(hidden, output_size_test))
...
## LOSE FUNCTION ################################
predquality = -tf.reduce_sum(tf.log(classification_train + 1e-5) * onehot_labels_train, 1) # cross-entropy
predcost = tf.reduce_mean(predquality)

correct_train = tf.arg_max(onehot_labels_train, 1)
prediction_train = tf.arg_max(classification_train, 1)
correct_test = tf.arg_max(onehot_labels_test, 1)
prediction_test = tf.arg_max(classification_test, 1)

# all-knower
R_train = tf.cast(tf.equal(correct_train, prediction_train), tf.float32)
reward_train = tf.reduce_mean(R_train)
R_test = tf.cast(tf.equal(correct_test, prediction_test), tf.float32)
reward_test = tf.reduce_mean(R_test)

def evaluate():
    data = load_input2.InputData()
    data.get_test(1, 1, min_blobs_test, max_blobs_test) # MT
    batches_in_epoch = len(data.images) // batch_size
    accuracy = 0
    sumlabels = np.zeros(output_size_test)
 
    for i in range(batches_in_epoch):
        nextX, nextY = data.next_batch(batch_size)
        sumlabels += np.sum(nextY, 0) 
        feed_dict = {x: nextX, onehot_labels_test: nextY}
        c, mr, iimg, r, pred, cor = sess.run([classification_test, maxr, rr, reward_test, prediction_test, correct_test], feed_dict=feed_dict)
        accuracy += r
...
if __name__ == '__main__':
    ...
    train_data = load_input2.InputData()
    train_data.get_train(None, None, min_blobs_train, max_blobs_train) # MT
    fetches2=[]
    fetches2.extend([reward_train, predcost, train_op])
    ...
    for i in range(start_restore_index, train_iters):
        ...
        results = sess.run(fetches2, feed_dict = {x: xtrain, onehot_labels_train: ytrain})

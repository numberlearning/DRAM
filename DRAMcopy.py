...
"translatedplain/classify_weird_from20000_log.csv",
"translatedplain/drawmodel20000.ckpt",
"translatedplain/classifymodel_weird_from_20000_",
"translatedplain/zzzdraw_data_5000.npy",
...
def linear(x,output_dim):
    ...
    # convert float32 tensor to int32 tensor
    result = tf.matmul(x, w) + b
    result = tf.to_int32(result, name='ToInt32')
    return result
...
def attn_window(scope,h_dec,N):
    ...
    # convert gx_ and gy_ to floats
    gx_ = tf.to_float(gx_, name='ToFloat')
    gy_ = tf.to_float(gy_, name='ToFloat')
    gx=(dims[0]+1)/2*(gx_+1)
    gy=(dims[1]+1)/2*(gy_+1)

    # convert log_sigma to float
    log_sigma2 = tf.to_float(log_sigma2, name='ToFloat')
    sigma2=tf.exp(log_sigma2)

    # convert log_delta to float
    log_delta = tf.to_float(log_delta, name='ToFloat')
    delta=(max(dims[0],dims[1])-1)/(N-1)*tf.exp(log_delta) # batch x N

    # convert log_gamma to float
    log_gamma = tf.to_float(log_gamma, name='ToFloat')
...
def read(x,h_dec_prev):
    ...
    def filter_img(img,Fx,Fy,gamma,N):
        ...
        print("Fxt.shape:", Fxt.shape)

        # changing the shape of Fxt so 0th dimensions match
        Fxt=tf.reshape(Fxt,[dims[1], 1, -1])
        # print("Fxt.shape:", Fxt.shape)

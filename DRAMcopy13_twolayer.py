...
def attn_window(scope,h_dec,N,glimpse):
    with tf.variable_scope(scope,reuse=REUSE):
        params=linear(h_dec,3) # batch_size x 3
    ...
    return Fx_1, Fy_1, Fx_2, Fy_2, gamma, gx, gy
...
def read(x, h_dec_prev, glimpse):
    Fx_1, Fy_1, Fx_2, Fy_2, gamma, gx, gy = attn_window("read", h_dec_prev, read_n, glimpse)
    stats = Fx_1, Fy_1, Fx_2, Fy_2, gamma
    new_stats = gx, gy
    ...
    def filter_img(img, Fx_1, Fy_1, Fx_2, Fy_2, gamma, N):
        ...
        glimpse_1 = tf.reshape(glimpse_1,[-1, N*N]) * tf.reshape(gamma, [-1,1])
        glimpse_2 = tf.matmul(Fy_2, tf.matmul(img, Fxt_2))
        glimpse_2 = tf.reshape(glimpse_2,[-1, N*N]) * tf.reshape(gamma, [-1,1])
        glimpse = tf.concat([glimpse_1, glimpse_2], 1) 
        return glimpse_1, glimpse_2, glimpse

    xr_1, xr_2, xr = filter_img(x, Fx_1, Fy_1, Fx_2, Fy_2, gamma, read_n) # batch_size x (read_n*read_n)
    return xr_1, xr_2, xr, new_stats # concat along feature axis
...
for glimpse in range(glimpses):
    r_1, r_2, r, stats = read(x, h_dec_prev, glimpse)

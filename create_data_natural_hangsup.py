    ...
    while (num_blobs < max_blobs + 1):
        ...
        while (i < nOfItem):
            ...
            while num_count < num_blobs:
                ...
                margin = 10
                radius = int(d_edge/2+1.5)*num_blobs
                x_center = 50#random.randint(margin+radius, img_width-width-margin-radius)
                y_center = 50#random.randint(margin+radius, img_height-height-margin-radius)

                if density:
                    cX = random.randint(int(x_center-radius), int(x_center+radius))
                    cY = random.randint(int(y_center-radius), int(y_center+radius))
                else:
                ...
                while index < num_count:
                    if cX+width+margin <= used[index, 0] or used[index, 0]+margin+used[index, 2] <= cX or used[index, 1]+margin+used[index,3] <= cY or cY+height+margin<=used[index,1]: # check for no overlapping blobs
                        index = index + 1
                    else:
                        if density:
                            cX = random.randint(int(x_center-radius), int(x_center+radius))

...
def generate_data(even, min_blobs, max_blobs): # MT     
    ...
    blob_list = np.zeros([total, glimpses, 2]) # the first one is the center
    size_list = np.zeros([total, glimpses-1])
    mask_list = np.zeros([total, glimpses-1])
    num_list = np.zeros(total)
    count_word = np.zeros([total, glimpses-1, n_labels+1]) 
    num_blobs = min_blobs 
    img_count = 0
    ...
    while (num_blobs < max_blobs + 1):
        ...
        for i in range(nOfItem):
            ...
            blob_list[img_count, 0, 0] = img_width/2
            blob_list[img_count, 0, 1] = img_height/2
            ...
            while num_count < num_blobs: 
                ...
                if num_count == 0:
                    ...
                    cX = cX_prev + width + random.randint(4, 8) # 9 blobs
                ...
                size_list[img_count, glimpse_count-1] = height 
                count_word[img_count, glimpse_count-1, num_count + 1] = 1 # "I'm done!":[1,0,0,0,0], "one":[0,1,0,0,0], "two":[0,0,1,0,0] 
                cX_prev = cX

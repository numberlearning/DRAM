...
cell = tf.contrib.rnn.LSTMCell(h_size, state_is_tuple=True) # LSTM
...
def attn_window(scope, blob_list, h, N, glimpse, gx_prev, gy_prev, testing=None): 
    with tf.variable_scope(scope,reuse=REUSE):
        params=linear(h,3) # batch_size x 3 
...
## READ ## 
def read(x, h_prev, glimpse, testing=None):
    Fx_1, Fy_1, Fx_2, Fy_2, gamma, gx, gy, gx_real, gy_real = attn_window("read", blob_list, h_prev, read_n, glimpse, gx_prev, gy_prev, testing)
...
def lstm_cell(input, state):
    ...
    with tf.variable_scope("LSTMCell", reuse=REUSE):
...
h_prev = tf.zeros((batch_size, h_size))
h_state = cell.zero_state(batch_size, tf.float32)
...
for glimpse in range(glimpses):
    r, stats = read(x, h_prev, glimpse, testing)
    predict_gx, predict_gy = stats
    target_gx = blob_list[0][glimpse][0]
    target_gy = blob_list[0][glimpse][1] 
    h, h_state = lstm_cell(tf.concat([r, h_prev], 1), h_state)
    h_prev = h

    with tf.variable_scope("output",reuse=REUSE):
        classification = tf.nn.softmax(linear(h, output_size + 1)) # add "I'm done!" tensor
    ...
    potquality = tf.where(in_blob, intensity/intensity, intensity)
...
def evaluate():
    ...
    for i in range(batches_in_epoch):
        ...
        r, cnt, cor, potx, poty, tch = sess.run([reward, counts, corrects, pointerxs, pointerys, teachers], feed_dict=feed_dict)
        accuracy += r

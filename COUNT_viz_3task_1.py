...
def attn_window(scope, blob_list, h_point, N, glimpse, gx_prev, gy_prev, testing): 
    with tf.variable_scope(scope,reuse=REUSE):
        params=linear(h_point,2) # batch_size x 3 
...
for glimpse in range(glimpses):
    ...
    task_str = tf.reshape(tf.cast([True, False, False], tf.float32), [batch_size, -1])
...
if __name__ == '__main__':
    ...
    fetches2=[]
    fetches2.extend([xxs, yys, blob_point, pointxs, pointys, predictxs, predictys, counts, corrects, count_accuracy, point_accuracy, predcost, train_op])
    ...
    for i in range(start_restore_index, train_iters):
        ...
        xs_fetched, ys_fetched, blbs_fetched, potxs_fetched, potys_fetched, prdxs_fetched, prdys_fetched, counts_fetched, corrects_fetched, count_accuracy_fetched, point_accuracy_fetched, predcost_fetched, _ = results

        # average over 100 batches
        sum_cnt_accuracy += count_accuracy_fetched 
        ...
        if i%300==0 or (i-1)%300==0 or (i-2)%300==0:
            if i==0 or i==1 or i==3:
                if i==0:
                    print("iter=%d" % (i))
                    print("TASK1: cnt_accuracy: %f, Pc: %f" % (sum_cnt_accuracy, sum_pc))
                    #print("PRED_X, TCH_X: " + str(xs_fetched)) 
                elif i==1: 
                    print("TASK2: pot_accuracy: %f, Pc: %f" % (sum_pot_accuracy, sum_pc))
                    #print("PRED_X, TCH_X: " + str(xs_fetched)) 
                else: 
                    print("TASK3: cnt_accuracy: %f, pot_accuracy: %f, Pc: %f" % (sum_cnt_accuracy, sum_pot_accuracy, sum_pc))
                    #print("PRED_X, TCH_X: " + str(xs_fetched)) 
              
            elif i%300==0:
                print("iter=%d" % (i//3))
                print("TASK1: cnt_accuracy: %f, Pc: %f" % (sum_cnt_accuracy/100, sum_pc/100))
                #print("PRED_X, TCH_X: " + str(xs_fetched)) 
            elif (i-1)%300==0:
                print("TASK2: pot_accuracy: %f, Pc: %f" % (sum_pot_accuracy/100, sum_pc/100))
                #print("PRED_X, TCH_X: " + str(xs_fetched)) 
            else:
                print("TASK3: cnt_accuracy: %f, pot_accuracy: %f, Pc: %f" % (sum_cnt_accuracy/100, sum_pot_accuracy/100, sum_pc/100))
                #print("PRED_X, TCH_X: " + str(xs_fetched)) 
        ...
        if (i-2)/3 in [0, 100, 200, 300, 400, 600, 800, 1200, 1600, 2400, 3200, 4800, 6400, 9600, 12800, 19200, 25600, 38400, 51200, 76800, 102400, 153600, 204800, 307200, 409600, 614400, 819200, 1000000, 1228800, 1638400, 2000000, 2457600, 3000000, 3276800, 4000000, 4915200, 5000000, 6000000, 6553600, 7000000]:

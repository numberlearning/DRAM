...
def get_dims(even, min_edge, max_edge):
    ...
    if even:
        ...
        width = 2*random.randint(1,3)+1 # 3,5,7 odd number
...
def get_center(even):
    """Get the parameters of the center for each blob"""
    ...
    while (num_blobs < max_blobs + 1):
        ...
        for i in range(nOfItem):
            ...
            while num_count < num_blobs: 
                ...
                cX_, cY_ = get_center(even)
                if num_count == 0:
                    cX = cX_  
                else:
                    cX = cX_prev + cX_ 
                cY = random.randint(img_height/2 - cY_, img_height/2 + cY_)-height//2
                blob_list[img_count, glimpse_count, 0] = cX + width//2

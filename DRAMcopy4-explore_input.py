...
def evaluate():
    ...
    print("type(data._images): ", type(data._images))
    print("data._images: ", data._images)
    accuracy = 0
...
if classify:
    ...
    for i in range(train_iters):
        ...
        print("len(xtrain): ", len(xtrain)) # batch_size
        print("type(ytrain): ", type(ytrain))
        print("len(ytrain): ", len(ytrain)) # batch_size
        if i == 0:
            print("xtrain")
            print(xtrain)
            for img_idx, img in enumerate(xtrain):
                ...
                image_matrix = list(chunks(img, 28))
                plt.imshow(image_matrix, interpolation="nearest", origin="upper")
                plt.colorbar()
                plt.show()
            ...
            print("ytrain")
            print(ytrain)
            for img_idx, img in enumerate(ytrain):
                print("label #", img_idx, " ", img)
        if translated:

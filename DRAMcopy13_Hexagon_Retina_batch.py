...
# delta, sigma2, mu, a, b 
pdelta=np.logspace(0, read_n-4, read_n-3, base=1.145)
tdelta=pow(9,pdelta)
tdelta = np.concatenate(((0,3,6),tdelta),axis=0) # tdelta[9]=141.34, 100*sqrt(2)=141.42
delta=np.zeros(read_n)
for i in range(1,read_n):
...
def read(x, h_dec_prev, glimpse):
    ...
    stats = Fx, Fy, mu_x, mu_y, gamma, gx, gy = att_ret[0]

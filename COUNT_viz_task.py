...
## STATE VARIABLES ##############
# initial states
gx_prev = tf.ones((batch_size, 1))*1#dims[0]/2
...
for glimpse in range(glimpses):
    ...
    task_str = tf.reshape(tf.cast([False, False, True], tf.float32), [batch_size, -1])
...
def evaluate():
    ...
    for i in range(batches_in_epoch):
        ...
        feed_dict = {x: nextX, onehot_labels: nextY, blob_list: nextZ, size_list: nextS, mask_list: nextM, num_list: nextN, count_word: nextC}
...
if __name__ == '__main__':
    ...
    for i in range(start_restore_index, train_iters):
        ...
        results = sess.run(fetches2, feed_dict = {x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        #if i%3==0 and i%2==0:#point_accuracy_prev < threshold:
        ...
        #    results = sess.run(fetches2, feed_dict = {task: [False, False, True], testing: False, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})
        #else:
        #    results = sess.run(fetches2, feed_dict = {task: [False, False, True], testing: True, x: xtrain, onehot_labels: ytrain, blob_list: ztrain, size_list: strain, mask_list: mtrain, num_list: ntrain, count_word: ctrain})

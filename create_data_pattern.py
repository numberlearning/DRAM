...
def generate_data(even, min_blobs, max_blobs): # MT
    ...
    while (num_blobs < max_blobs + 1):
        ...
        while (i < nOfItem):
            img = np.ones(img_height*img_width) # input img size # change zeros to ones
            num_count = 0
            pattern_count = 0
            used = np.zeros((num_blobs, 4)) # check overlapping
        
            while (num_blobs == 5 and pattern_count < 50):
                height = min_edge
                width = height
                cX = random.randint(width+1, 99-width-1)
                cY = random.randint(width+1, 99-width-1)
                for p in range(cY, cY+height): # center
                    for q in range(cX, cX+width):
                        img[p*img_width+q] = 255
                for p in range(cY-3, cY-3+height): # top left
                    for q in range(cX-3, cX-3+width):
                        img[p*img_width+q] = 255
                for p in range(cY-3, cY-3+height): # top right
                    for q in range(cX+3, cX+3+width):
                        img[p*img_width+q] = 255
                for p in range(cY+3, cY+3+height): # lower left
                    for q in range(cX-3, cX-3+width):
                        img[p*img_width+q] = 255
                for p in range(cY+3, cY+3+height): # lower right
                    for q in range(cX+3, cX+3+width):
                        img[p*img_width+q] = 255
                train[img_count] = img
                img_count += 1
            ...
            if min_blobs == 1:
                ...
                print("Hey, you! Min blobs should be one !!!!")
            img_count += 1
            i += 1

...
def attn_window(scope,h_dec,N, predx=None, predy=None, DO_SHARE=False):
    ...
    # when training, set the attention window center to be predx and predy
...
while next_index < glimpses:
    ...
    _, _, _, _, _, _, _, _, _, gx, gy = attn_window("read", h_dec, read_n, DO_SHARE=True)
    predict_x, predict_y = gx, gy
    ...
    #reward_cnt = tf.reduce_sum(cnt_precision, 1) reward_count_list.append(reward_cnt)

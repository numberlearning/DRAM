...
log_filename = "translatedplain/copyglimpse_from_trainonlocssparse1000_1000_log.csv"
load_file = "translatedplain/copyglimpse_from_trainonlocssparse1000_1000.ckpt"
save_file = "translatedplain/classify_copyglimpse_from_trainonlocssparse1000_1000_"
...
with tf.variable_scope("gZ", reuse=None):
    ...
    for t in range(T):
        ...
        param_diff = param_diff + tf.reduce_mean(tf.square(g_params - params_hist[t]))
...
g_cost = param_diff + 0.001 * tf.reduce_sum(tf.abs(h_dec))
...
fetches2=[]
fetches2.extend([reward, param_diff, loc_dist, train_op2])
...
for i in range(train_iters):
    ...
    reward_fetched, param_diff_fetched, loc_dist_fetched, _=results
    if i%100==0:
        ...
        if i %1000==0:
            ...
            saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.VARIABLES, scope='gZ')) # saves variables learned during training

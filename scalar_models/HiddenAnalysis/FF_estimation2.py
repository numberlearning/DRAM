...
mask1 = tf.placeholder(tf.float32, shape=(h_size1))
mask2 = tf.placeholder(tf.float32, shape=(h_size2))
...
with tf.variable_scope("hidden1",reuse=REUSE):
    ...
    hidden1 = tf.nn.relu(linear(r, h_size1))
    masked_hidden1 = tf.multiply(hidden1, mask1)
    masked_hidden1 = tf.reshape(masked_hidden1, [batch_size, h_size1])
    hiddens1.append({
        "hidden1":hidden1,
...
with tf.variable_scope("hidden2",reuse=REUSE):
    #hidden = tf.nn.leaky_relu(linear(r, h_size))
    hidden2 = tf.nn.relu(linear(hidden1, h_size2))
    masked_hidden2 = tf.multiply(hidden2, mask2)
    masked_hidden2 = tf.reshape(masked_hidden2, [batch_size, h_size2])
    hiddens2.append({
        "hidden2":hidden2,
        "masked_hidden2":masked_hidden2,
        })
with tf.variable_scope("output",reuse=tf.AUTO_REUSE):
    classification = tf.nn.softmax(linear(hidden2, output_size))
    raw_output = linear(hidden2, output_size)
    masked_raw_output = linear(masked_hidden2, output_size)
    masked_classification = tf.nn.softmax(linear(masked_hidden2, output_size))

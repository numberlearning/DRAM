...
def get_k(testing=False):
    ...
    if testing:
        ...
        #return 3  # CAA
        return random.randint(2, 4) # for both new CAA & DAA
...
def get_s(testing=False, n=None):
    ...
    if testing:
        if n is not None:
            return random.uniform(.8*3 * (n ** (-.5)), 1.2*3 * (n ** (-.5))) # CTA
        else:
            return random.uniform(.8, 1.2) # CAA
    else:
        return random.uniform(.5, 1.5) # CAA
...
def generate_data(testing, min_blobs, max_blobs): # MT
    ...
    while (num_blobs < max_blobs + 1):
        ...
        while (i < nOfItem):
            ...
            total_area = 0.0 # total area in an image
            ...
            while num_count < num_blobs:
                ...
                total_area += width * height # add area of a blob
